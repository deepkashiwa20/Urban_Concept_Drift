...
def trainModel(mode, XS, YS):
    ...
    # Replay Memory
    replay_memory = torch.randn((XS.shape[0], encoder_dim, n_node, channel), dtype=torch.float).to(device)
    ...
    for epoch in range(EPOCH):
        ...
        for idx, (x, y) in train_iter:
            ...
            optimizer.zero_grad()
            y_pred, new_values = model(x, values)
            replay_memory_tmp = update_value(idx, new_values, replay_memory_tmp, mem_start=0)  # Update Replay Memory
...
def testModel(mode, XS, YS):
    ...
    replay_memory = torch.cat([replay_memory_his, replay_memory_holder], dim=0).to(device)
